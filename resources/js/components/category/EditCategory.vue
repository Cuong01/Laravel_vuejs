<template>
    <form @submit.prevent="editBrand()">
        <div class="container">
            ...
            <div class="mt-4">
                <button class="btn btn-info">Sửa danh mục</button>
...
const editBrand = async () => {
    await axios
        .post("/api/edit_category/" + route.params.id, Category)
...
const showBrand = async () => {
    await axios
        .get("/api/show_category/" + route.params.id)
        .then(function (response) {
            Category.name = response.data.brand.name;
            Category.status = response.data.brand.status;
        });
};
</script>
